...
public partial class showReportsNew : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs args)
    {
        ...
        if (Application["facultyCategoryID"] == null)
        ...
        if (!this.IsPostBack)
            if (specificQuery == false || reportNo != "7")
        ...
        using (SqlConnection con = new SqlConnection(Application["constr"].ToString()))
        {
            using (SqlCommand cmd = new SqlCommand(reportQuery))
            {
                ...
                if (reportNo == "7")
                {
                    ...
                    cmd.Parameters.AddWithValue("@var", Application["facultyCategoryID"].ToString());
